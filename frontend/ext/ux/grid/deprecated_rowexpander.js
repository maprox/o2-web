...
/**
	* @cfg {Boolean} hideExpanderIcon
	* <tt>true</tt> to hide expander button
	* (defaults to <tt>false</tt>).
	*/
...
/**
	...
	*/
	...
	constructor: function() {
		...
		// </debug>
		var rowBodyTpl = null;
		if (this.rowBodyTpl instanceof Ext.XTemplate) {
	...
	toggleRow: function(rowIdx) {
		...
		if (!grid.isFixedHeight ||
			(grid.isFixedHeight && !grid.isFixedHeight())) {
			grid.doComponentLayout();
		}
		this.view.up('gridpanel').invalidateScroller();
		//this.view.up('gridpanel').showVerticalScroller();
	},
	...
	onDblClick: function(view, record, htmlItem, index, e, eOpts) {
		if (this.expandable(record)) {
			this.toggleRow(htmlItem);
		}
	},

	onClick: function(view, record, htmlItem, index, e, eOpts) {
		if (this.expandable(record)) {
			this.toggleRow(htmlItem);
		}
	},
