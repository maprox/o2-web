/**
 *
 * Device editor
 * @class O.x.act.group_mon_device.Module
 * @extends C.ui.Module
 */
Ext.define('O.x.act.group_mon_device.Module', {
	extend: 'C.ui.Module',

/**
	* Alias string for this module
	* @important "alias" is needed by Ext.ClassManager to find module class
	* @type String
	*/
	alias: 'module-x-group-mon-device',

/**
	* Module identifier
	* Is used for storing and searching the module by its name
	* @type String
	*/
	id: 'x-group-mon-device',

/**
	* Module type
	*/
	type: 'module',

/**
	* Module weight
	*/
	weight: 10,

/**
	* xtype of a panel (for modules with 'config' or 'module' type)
	* @type String
	*/
	panel: 'x-group-mon-device',

/**
	* Short module name (for buttons, etc.)
	* @type String
	*/
	textShort: _('Device groups'),

/**
	* Short description of module (for tooltips, etc.)
	* @type String
	*/
	textLong: _('Device groups')

});