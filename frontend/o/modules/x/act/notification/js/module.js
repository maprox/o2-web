/**
 *
 * Notification editor
 * @class O.x.act.notification.Module
 * @extends C.ui.Module
 */
C.define('O.x.act.notification.Module', {
	extend: 'C.ui.Module',

/**
	* Alias string for this module
	* @important "alias" is needed by Ext.ClassManager to find module class
	* @type String
	*/
	alias: 'module-x-notification',

/**
	* Module identifier
	* Is used for storing and searching the module by its name
	* @type String
	*/
	id: 'x-notification',

/**
	* Module type
	*/
	type: 'module',

/**
	* Module weight
	*/
	weight: 30,

/**
	* xtype of a panel (for modules with 'config' or 'module' type)
	* @type String
	*/
	panel: 'x-notification',

/**
	* Short module name (for buttons, etc.)
	* @type String
	*/
	textShort: _('Notifications'),

/**
	* Short description of module (for tooltips, etc.)
	* @type String
	*/
	textLong: _('Notifications')

});