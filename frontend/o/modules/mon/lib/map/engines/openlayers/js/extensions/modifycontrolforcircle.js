/*
 * Собственный Control редактирования кругов, ничего подобного стандартного нет
 *
 */
if (C.utils.isset('C.lib.map.openlayers.Extender')) {
	C.lib.map.openlayers.Extender.add(function() {
		OpenLayers.Control.ModifyFeatureForCircle =
     ...
     */
			initialize: function(layer, options, center, coords) {
				this.layer = layer;
				this.vertices = [];
				...
				this.handler = new OpenLayers.Handler.CustomRegularPolygon(this,
					this.callbacks, this.handlerOptions);
				this.handler.setCenter(center.lat, center.lon);
				this.coords = coords;
			...
			editMode: function() {
				this.activate();
				this.handler.restoreGeometry(this.coords);
			},

			CLASS_NAME: "OpenLayers.Control.ModifyFeatureForCircle"
		});
	});
}
